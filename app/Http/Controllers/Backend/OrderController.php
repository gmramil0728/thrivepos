...
class OrderController extends Controller
{
    ...
    $product = Orderdetails::where('order_id',$order_id)->get();
        foreach($product as $item){
           Product::where('id',$item->product_id)
                ->update(['inventory_count' => DB::raw('inventory_count-'.$item->quantity) ]);
        }
    ...
    public function OrderInvoice($order_id){
        ...
        $pdf = Pdf::loadView('backend.order.order_invoice', compact('order','orderItem','company'))->setPaper('a4')->setOption([
                'tempDir' => public_path(),
                'chroot' => public_path(),
    ...
    public function UpdateDue(Request $request)
    {
        ...
        // Server-side Math
        // $new_payment_amount = $request->pay;
        $new_payment_amount = $request->cash_received;
        $updated_due = max(0, $order->due - $new_payment_amount);
        $updated_paid = $order->pay + $new_payment_amount;

        // Determine Statuses
        $is_fully_paid = ($updated_due <= 0);
        $payment_status = $is_fully_paid ? 'paid' : 'partial';
        $order_status = $is_fully_paid ? 'complete' : $order->order_status;

        // 1. Record the Payment History (The Ledger)
        OrderPayment::create([
            'order_id' => $order->id,
        ...
        $notification = [
            'message' => $is_fully_paid 
                ...
                : 'Partial Payment of ₱' . number_format($new_payment_amount, 2) . ' recorded.',
            'alert-type' => 'success'
        ];
